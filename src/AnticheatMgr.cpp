...
void AnticheatMgr::JumpHackDetection(Player* player, MovementInfo /* movementInfo */, uint32 opcode)
{
	...
	if (m_Players[key].GetLastOpcode() == MSG_MOVE_JUMP && opcode == MSG_MOVE_JUMP)
	{
		...
		sLog->outString("AnticheatMgr:: Jump-Hack 检测到 玩家 %s (%s)", player->GetName().c_str(), player->GetGUID().ToString().c_str());
	}
}
...
void AnticheatMgr::WalkOnWaterHackDetection(Player* player, MovementInfo  movementInfo)
{
	...
	if (sConfigMgr->GetBoolDefault("Anticheat.KickPlayerWaterWalkHack", false))
	{
		if (sConfigMgr->GetBoolDefault("Anticheat.WriteLog", false))
			sLog->outString("AnticheatMgr:: 在水中行走-被发现并被踢玩家 %s (%s)", player->GetName().c_str(), player->GetGUID().ToString().c_str());
		...
		sLog->outString("AnticheatMgr:: 水中行走-被发现玩家 %s (%s)", player->GetName().c_str(), player->GetGUID().ToString().c_str());
	}
...
void AnticheatMgr::FlyHackDetection(Player* player, MovementInfo  movementInfo)
{
    ...
    if (sConfigMgr->GetBoolDefault("Anticheat.KickPlayerFlyHack", false))
    {
        if (sConfigMgr->GetBoolDefault("Anticheat.WriteLog", false))
        {
            sLog->outString("AnticheatMgr:: 飞天-被发现并被踢玩家 %s (%s)", player->GetName().c_str(), player->GetGUID().ToString().c_str());
        }
      ...
      {
        sLog->outString( "AnticheatMgr:: 飞天-被发现的玩家 %s (%s)", player->GetName().c_str(), player->GetGUID().ToString().c_str());
      }
...
void AnticheatMgr::TeleportPlaneHackDetection(Player* player, MovementInfo movementInfo)
{
	...
	{
		if (sConfigMgr->GetBoolDefault("Anticheat.WriteLog", false))
			sLog->outString("AnticheatMgr:: 非法传送-检测到玩家 %s (%s)", player->GetName().c_str(), player->GetGUID().ToString().c_str());
...
void AnticheatMgr::ClimbHackDetection(Player *player, MovementInfo movementInfo, uint32 opcode)
{
	...
	{
		if (sConfigMgr->GetBoolDefault("Anticheat.WriteLog", false))
			sLog->outString("AnticheatMgr:: 爬升-检测到玩家 %s (%s)", player->GetName().c_str(), player->GetGUID().ToString().c_str());
...
void AnticheatMgr::SpeedHackDetection(Player* player, MovementInfo movementInfo)
{
	...
	{
		if (sConfigMgr->GetBoolDefault("Anticheat.WriteLog", false))
			sLog->outString("AnticheatMgr:: 加速-检测到玩家 %s (%s)", player->GetName().c_str(), player->GetGUID().ToString().c_str());
